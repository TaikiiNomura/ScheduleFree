...
def plot_loss_accuracy(
    ...
    for name, info in optimizer_info.items():
        plt.plot(range(1, num_epochs + 1), all_train_losses[name], label=info['label'], color=info['color'])
    plt.xlabel('Epoch')
    ...
    plt.subplot(2, 2, 3)
    for name, info in optimizer_info.items():
        plt.plot(range(1, num_epochs + 1), all_train_accuracies[name], label=info['label'], color=info['color'])
    plt.xlabel('Epoch')
